...
// Copyright (c) 2018 The Cream developers www.creamcoin.com
 ...
 * between creamd, and cream-qt and non-server tools.
*/
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/creamcoin/cream>";
    const std::string URL_WEBSITE = "<https://creamcoin.com>";
...
/** Sanity checks
 *  Ensure that Cream is running in a usable environment with all
...
{
    // Make sure only a single Cream process is using the data directory.
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if cream is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if cream is started while in a temporary directory.\n",
